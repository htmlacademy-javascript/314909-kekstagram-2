...
Удалён обработчик события reset, который ранее вызывал функцию закрытия формы.
В предыдущей реализации это приводило к повторному вызову form.reset() внутри closeForm(), что могло вызывать некорректное поведение.
